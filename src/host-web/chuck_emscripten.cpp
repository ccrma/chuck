...
extern "C"
{
    ...
    // main loop (must be started AFTER chugins have loaded)
    static void EMSCRIPTEN_KEEPALIVE runMainThreadHook(Chuck_DL_MainThreadHook* hook)
    {
        return;
        if (hook) {
            hook->m_hook(hook->m_bindle); // blocking
            hook->m_quit(hook->m_bindle);
        }
    }

    // something to do nothing
    void EMSCRIPTEN_KEEPALIVE dummyFunction()
    {
                                                ...
                                                unsigned int inChannels, unsigned int outChannels)
    {
        ...
        if (chuck_instances.count(chuckID) == 0)
        {
            ...
            chuck->init(); // chugins initialized here
            chuck->start();

            // put into map
            chuck_instances[chuckID] = chuck;

            // start main loop on separate thread
            // std::thread main_thread(runMainThreadHook, chuck->getMainThreadHook());
            std::thread main_thread(dummyFunction);
            // std::thread main_thread(runMainThreadHook, NULL);
            main_thread.detach();
    ...
    void EMSCRIPTEN_KEEPALIVE cleanRegisteredChucks()
    {
        ...
        for (std::map<unsigned int, ChucK *>::iterator it =
                 chuck_instances.begin();
             it != chuck_instances.end(); it++)
        {
