...
{
    Chuck_String *type = (Chuck_String *) instantiate_and_initialize_object(SHRED->vm_ref->env_ref->t_string, SHRED);
    SAFE_ADD_REF(type);
    ...
    Chuck_String *s = (Chuck_String *) instantiate_and_initialize_object(SHRED->vm_ref->env_ref->t_string, SHRED);
    SAFE_ADD_REF(s);
...
CK_DLL_MFUN(oscin_recv)
{
    ...
    oscarg_type = type_engine_find_type( SHRED->vm_ref->env_ref, str2list("OscArg") );
...
{
    Chuck_String *address = (Chuck_String *) instantiate_and_initialize_object(SHRED->vm_ref->env_ref->t_string, SHRED);
    SAFE_ADD_REF(address);
    OBJ_MEMBER_STRING(SELF, oscmsg_offset_address) = address;
    
    Chuck_String *typetag = (Chuck_String *) instantiate_and_initialize_object(SHRED->vm_ref->env_ref->t_string, SHRED);
    SAFE_ADD_REF(typetag);
    OBJ_MEMBER_STRING(SELF, oscmsg_offset_typetag) = typetag;
    
    Chuck_Array4 *args = new Chuck_Array4(TRUE);
    initialize_object(args, SHRED->vm_ref->env_ref->t_array);
    args->clear();
    SAFE_ADD_REF(args);
...
CK_DLL_MFUN( osc_address_next_string  ) { 
    ...
    initialize_object( ckstr, SHRED->vm_ref->env_ref->t_string );
