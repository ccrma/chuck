...
t_CKBOOL Chuck_Compiler::do_entire_file( Chuck_Context * context )
{
    ...
    cout << "all unresolved types: " << env()->context->unresolved_types.size() << endl;
    ...
    if (!type_engine_handle_unresolved_types(env()))
      return FALSE;
                                         ...
                                         const char * str_src, const string & full_path )
{
    ...
    // reset the env (do I was to reset the env? How do I get back?)
    // env()->reset();
